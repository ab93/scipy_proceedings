...
label-free signal available. Reading them together is the point: the first
calibrates the claim, the second establishes it.
...
Every number, table, and figure below is regenerated by the two notebooks
accompanying this paper (`nbs/01_study_a_covariate_shift.ipynb` and
`nbs/02_study_b_version_regression.ipynb`), and identically by
...
This is worth stating plainly, because it bounds the claim. A deliberate
demographic shift is exactly the regime input monitoring was designed for, and
explanation drift adds no *global* detection power here. Taken alone, Study A
establishes the weaker of the available claims: that a model's attribution
behavior can change substantially while its accuracy does not.
...
**The per-feature view is where the two signals genuinely disagree.** @fig:psi
...
The reverse holds for the features the shift acted on directly: `age`
(11.07 vs. 20.74), `marital-status` (0.671 vs. 1.025), and `relationship`
(0.329 vs. 0.655) all move *more* in input space than in attribution space. This
is what one should expect, and it is why the two views are complements rather
than substitutes. In the no-drift control, the largest attribution PSI across all
fourteen features is 0.0079, comfortably inside the stable band.
...
Looking inside the most-drifted feature confirms the mechanism. @fig:dist overlays
the `age` attribution distribution in the two periods: broad and bimodal in the
reference period, collapsing under the shift into a tight, strongly positive
cluster, so that for the older population `age` has become a consistent large
positive driver of the high-income prediction. `shap-monitor` additionally reports
a +33% change in `age` mean absolute attribution, +82% for `workclass`, +51% for
`capital-gain`, and sign flips on six features including `marital-status` and
`workclass`, meaning their typical contribution direction reversed. None of this
